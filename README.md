...
基于 Qt+OpenCV+YOLOv8 构建的智能安防监控系统，采用 C/S 架构与多线程解耦设计。
...
- **视频录制**：实时视频录制与保存
- **历史查询**：支持查询历史抓拍图片与录制视频

### 🔒 安防功能
- **运动追踪**：基于背景建模的目标运动追踪
- **区域入侵**：自定义警戒区域，检测非法入侵行为
- **多客户端管理**：服务端可管理多个客户端连接
...
2. 修改 `server.pro` 和 `client.pro` 中的 OpenCV 路径：
```qmake
